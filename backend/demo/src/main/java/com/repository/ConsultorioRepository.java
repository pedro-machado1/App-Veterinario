...
public interface ConsultorioRepository extends JpaRepository<Consultorio, Long> {
    ...
    @Query("SELECT DISTINCT c FROM Consultorio c WHERE c.cidade.id = :cidadeId")
    Page<Consultorio> findAllByCidadeId(@Param("cidadeId") Long cidadeId, Pageable pageable);

    @Query("SELECT DISTINCT c FROM Consultorio c WHERE c.estado = :estado AND c.cidade.id = :cidadeId")
    ...
    @Query("SELECT DISTINCT cl FROM Consultorio c JOIN c.cliente cl WHERE c.id = :consultorioId AND cl.cidade.id = :cidadeId")
    ...
    @Query("SELECT DISTINCT cl FROM Consultorio c JOIN c.cliente cl WHERE c.id = :consultorioId AND cl.cpf LIKE CONCAT('%', :cpf, '%') AND cl.cidade.id = :cidadeId")
